...
public interface InferencePreparationService {

  InferenceContext prepareUsingExistingModel(ChatTurn chatTurn, InferenceDTO dto);

  InferenceContext prepareWithExplicitModel(
      ChatTurn chatTurn, InferenceDTO dto, String modelId, Boolean duplicateOnNoResponse);

  InferenceDTO createInferenceStatus(InferenceDTO dto);

  List<Prompt> buildPromptHistory(List<ChatTurn> previousTurns, ChatTurn currentTurn);
}
